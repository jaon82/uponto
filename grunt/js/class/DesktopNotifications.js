...
/**
 * Verifica se o browser oferece suporte para as notificações que o usário já concedeu permissão
...
DesktopNotifications.prototype.checkPermission = function(){
	...
	if (!("Notification" in window)) {
	  result.msg = "Este browser não oferece suporte para notificaçãoes de desktop.";
	  result.isOK = false;
	}
...
DesktopNotifications.prototype.create = function(title,options,url,sound){
	if(permission.isOK){
		...
		if(confirm("Voc� ainda não autorizou o uso de notificações.\nDeseja revisar as pemissões?")){
			permission = this.checkPermission();
		}
	}
};
