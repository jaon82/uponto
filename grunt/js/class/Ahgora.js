...
function Ahgora(matricula,senha,empresa,helpers,dao,openAlert,closeAlert,monthFunc,timerFunc){
	...
	if(matricula == null || senha == null || empresa == null){
		var msg = "Informe sua Matr�cula, Senha e Empresa nas Configurações antes de realizar a importação.";
...
Ahgora.prototype.checkSettings = function(){
	if(matricula == null || senha == null || empresa == null){
		var msg = "Informe sua Matrícula, Senha e Empresa nas Configuraçõess antes de realizar a importação.";
		$("#msg-lightbox-content").html(msg);
		closeLightbox("#perform-update");
		openLightbox("#msg-lightbox");
		return false;
...
Ahgora.prototype.import = function (month,year,day){
			...
			for(var i = 0; i <= resposta.length; i++){
				...
				for(var j in dados){
					...
					if(texto.length > 0){
						...
						//Se vai armazenar a importação por dia; ou
						//Se foi uma importação por dia, só vai alterar o dia selecionado
						day = parseInt(day);
						dia = parseInt(dia);
						...
						if(day == 0 || day == dia){
							...
							//Não vai sobrescrever registros que já existem, exceto quando se trata de uma atualização por dia.
	...
	}, "json").fail(function() {//Se ocorreu erro na requisi��o
		closeLightbox("#perform-update");
		openLightbox("#error-update");
